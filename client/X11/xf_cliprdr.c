                                                         ...
                                                         UINT32 dstFormatId)
{
	...
	ClipboardLock(clipboard->system);
	success = ClipboardSetData(clipboard->system, srcFormatId, cached_data->data,
	                           cached_data->data_length);
	if (!success)
	{
		WLog_WARN(TAG, "Failed to set clipboard data (formatId: %u, data: %p, data_length: %u)",
		          srcFormatId, cached_data->data, cached_data->data_length);
		ClipboardUnlock(clipboard->system);
		return NULL;
	}
			...
			if (!cached_data && !rawTransfer)
			{
				...
				if (cached_raw_data && cached_raw_data->data_length != 0)
					cached_data = convert_data_from_existing_raw_data(
					    clipboard, cached_raw_data, formatId, nullTerminated, dstFormatId);
			}
                                       ...
                                       const CLIPRDR_FORMAT_DATA_RESPONSE* formatDataResponse)
{
	...
	if (raw_data)
	{
		...
		cached_data = xf_cached_data_new(raw_data, size);
		if (!cached_data)
		{
			WLog_WARN(TAG, "Failed to allocate cache entry");
			free(raw_data);
		...
		{
			if (!HashTable_Insert(clipboard->cachedRawData,
			                      (void*)(UINT_PTR)clipboard->requestedFormatId, cached_data))
				WLog_WARN(TAG, "Failed to cache clipboard data");
		}
	}
	else
